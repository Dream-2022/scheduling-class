<template>
  <div class="wow animate__fadeInUp white-box">
    <div class="middle-box">
      <div class="title-box">
        <div class="middle-title">近日课程安排</div>
      ...
      <div class="timeline-container">
        <el-timeline>
          ...
          >
            <el-card class="timeline-card" v-if="getWeekDay(day.day) != '周二'">
              <template #header>
  ...
  <el-drawer v-model="detailVisible" :show-close="false" size="500px">
    ...
    <div class="course-detail" v-if="currentCourse">
      <div class="detail-section">
        ...
        <div class="detail-content">
          ...
          <div class="detail-item">
            ...
            <span class="detail-value">{{ currentCourse.teacherName }}</span>
          </div>
          <div class="detail-item">
      ...
      <div class="detail-section">
        <div class="detail-title">课程作业</div>
        <div class="detail-content">
          <div class="homework-list">
            <div v-for="(homework, index) in courseHomework" :key="index" class="homework-item">
              <div class="homework-header">
                <span class="homework-title">{{ homework.title }}</span>
                <el-tag
                  size="small"
                  :type="homework.status === 'completed' ? 'success' : 'warning'"
                >
                  {{ homework.status === 'completed' ? '已完成' : '待完成' }}
                </el-tag>
              </div>
              <div class="homework-desc">{{ homework.description }}</div>
              <div class="homework-footer">
                <span class="homework-deadline">截止日期: {{ homework.deadline }}</span>
                <el-button
                  type="primary"
                  size="small"
                ...
                >
                  {{ homework.status === 'completed' ? '已提交' : '提交作业' }}
                </el-button>
              </div>
            </div>
          </div>
          <el-empty v-if="courseHomework.length === 0" description="暂无课程作业" />
        </div>
      </div>
    </div>
  </el-drawer>
...
const courseHomework = ref([
  {
    title: '高等数学第一章习题',
    description: '完成教材P45-P46的习题1-10',
    deadline: '2023-10-15',
    status: 'pending',
  },
  {
    title: '大学英语口语练习',
    description: '准备3分钟自我介绍，录制视频上传',
    deadline: '2023-10-20',
    status: 'completed',
  },
  {
    title: '计算机导论实验报告',
    description: '完成实验一：计算机硬件认识与组装',
    deadline: '2023-10-25',
    status: 'pending',
  },
])
...
const submitHomework = homework => {
  ElMessage.success(`作业"${homework.title}"提交成功`)
