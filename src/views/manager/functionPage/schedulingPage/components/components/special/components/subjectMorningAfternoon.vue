<template>
  ...
  <el-dialog v-model="dialogVisible" title="新增科目上下午限制" width="500">
    <el-form
      ref="ruleFormRef"
    ...
    >
      ...
      <el-form-item label="限制时段" prop="period">
        <el-radio-group v-model="addSubjectForm.period">
...
const subjectList = ref(props.allContent.subjectMorningAfternoon || [])
...
onMounted(async () => {
  ...
  await Promise.all([getSubjectOptions()])
})
...
function addClick() {
  if (!addSubjectForm.value.subjects?.length) {
...
const rules = ref({
  ...
  period: [{ required: true, message: '请选择限制时段', trigger: 'blur' }],
})
...
watch(
  ...
  newValue => {
    emit('update:allContent', {
      ...props.allContent,
      subjectMorningAfternoon: newValue,
    })
  },
  { deep: true },
)
