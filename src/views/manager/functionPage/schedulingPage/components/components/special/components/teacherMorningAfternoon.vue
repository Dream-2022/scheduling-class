<template>
  ...
  <el-dialog v-model="dialogVisible" title="新增教师上下午限制" width="500">
    <el-form
      ref="ruleFormRef"
    ...
    >
      ...
      <el-form-item label="限制时段" prop="period">
        <el-radio-group v-model="addTeacherForm.period">
...
const teacherList = ref(props.allContent.teacherMorningAfternoon || [])
...
onMounted(async () => {
  ...
  await Promise.all([getTeacherOptions()])
})
...
watch(
  ...
  newValue => {
    emit('update:allContent', {
      ...props.allContent,
      teacherMorningAfternoon: newValue,
    })
  },
  { deep: true },
)
