<template>
  ...
  <el-dialog v-model="dialogVisible" title="新增教师限制" width="500">
    <el-form
      ref="ruleFormRef"
...
const teacherList = ref(props.allContent.teacherDayLimit || [])
...
const handleSearch = () => {
  if (!selectedArr.value || selectedArr.value.length === 0) {
    teacherList.value = props.allContent.teacherDayLimit || []
    return
  }

  teacherList.value = (props.allContent.teacherDayLimit || []).filter(item => {
