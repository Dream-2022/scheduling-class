<template>
          ...
          <div class="activity-box-left">
            ...
            <div class="teacher-info">
              ...
              <div class="limit-info">
                每周课时 <span class="limit-num">{{ item.minPerWeek }}</span> -
  ...
  <el-dialog v-model="dialogVisible" title="新增教师周课时限制" width="500">
    <el-form
      ref="ruleFormRef"
    ...
    >
      ...
      <div style="display: flex; gap: 10px">
        <el-form-item label="每周课时" prop="minPerWeek">
          <el-input-number
            style="width: 140px"
            v-model="addTeacherForm.minPerWeek"
            :min="1"
            :max="40"
            placeholder="最少课时"
          />
        </el-form-item>
        <el-form-item label="至" prop="maxPerWeek">
          <el-input-number
            style="width: 140px"
...
const teacherList = ref(props.allContent.teacherWeekLimit || [])
...
function addClick() {
  if (!addTeacherForm.value.teachers || addTeacherForm.value.teachers.length === 0) {
    ElMessage.warning('请选择教师')
    return
  }

  if (addTeacherForm.value.minPerWeek > addTeacherForm.value.maxPerWeek) {
    ElMessage.warning('最少课时不能大于最多课时')
    return
  }
...
const handleSearch = () => {
  if (!selectedArr.value || selectedArr.value.length === 0) {
    teacherList.value = props.allContent.teacherWeekLimit || []
    return
  }

  teacherList.value = (props.allContent.teacherWeekLimit || []).filter(item => {
...
const rules = ref({
  ...
  minPerWeek: [{ required: true, message: '请输入最少课时', trigger: 'blur' }],
  maxPerWeek: [{ required: true, message: '请输入最多课时', trigger: 'blur' }],
})
