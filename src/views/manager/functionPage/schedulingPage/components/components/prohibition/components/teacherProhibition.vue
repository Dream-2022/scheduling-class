...
const teacherList = ref(props.allContent.teacherProhibition || [])
...
const handleSearch = () => {
  if (!selectedArr.value || selectedArr.value.length === 0) {
    teacherList.value = props.allContent.teacherProhibition || []
    return
  }

  teacherList.value = (props.allContent.teacherProhibition || []).filter(item => {
