<template>
  <div class="priority-time">
    <div class="wow animate__fadeInLeft warn-box">
      ...
      优先级时间片：不同优先级的课程可以设置在不同的时间片优先排课，高优先级的时间片将会优先被使用
    </div>
...
const priorityList = ref(
  props.allContent.creditHourType || {
    lowPriority: [],
    middlePriority: [],
    highPriority: [],
  },
...
onMounted(() => {
  // 生成时间片数据
  const times = ['实践', '实验', '理论', '上机', '其他']
  timeList.value = times.map(item => ({
    label: item,
    key: item,
    initial: item,
  }))
  priorityList.value.lowPriority = timeList.value.map(item => item.key)
