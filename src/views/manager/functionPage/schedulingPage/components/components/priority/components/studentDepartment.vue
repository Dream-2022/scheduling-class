<template>
  <div class="student-department">
    <div class="wow animate__fadeInLeft warn-box">
      ...
      学生院系优先：优先级更高院系的学生将会优先排课，可以搭配优先级时间片（不同优先级院系可以设置不同时间片优先排课）
    </div>
    ...
    <div class="priority-panels">
      ...
      <div class="priority-panel">
        <div class="panel-header">
          ...
          <el-tag type="info" class="count-tag">{{ filteredLowPriorityData.length }}</el-tag>
        </div>
        <el-input
          v-model="searchLow"
          placeholder="搜索院系"
      ...
      <div class="priority-panel">
        <div class="panel-header">
          ...
          <el-tag type="success" class="count-tag">{{ filteredHighPriorityData.length }}</el-tag>
        </div>
        <el-input
          v-model="searchHigh"
          style="width: 410px"
...
const priorityList = ref(
  props.allContent.studentDepartment || {
    lowPriority: [],
    middlePriority: [],
    highPriority: [],
  },
...
onMounted(async () => {
  ...
  if (
    !props.allContent.studentDepartment ||
    !props.allContent.studentDepartment.lowPriority ||
    !props.allContent.studentDepartment.lowPriority.length
  ) {
...
const moveToMiddle = () => {
  if (
    !priorityList.value?.lowPriority ||
    !priorityList.value?.highPriority ||
    !priorityList.value?.middlePriority
  ) {
    priorityList.value = {
